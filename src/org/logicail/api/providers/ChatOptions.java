 ...
 * Time: 11:45
 */
public class ChatOptions extends LogicailMethodProvider {
	...
	public ChatOptions(LogicailMethodContext context) {
		super(context);
	}
	...
	/*public static ChatOption getContinueOption() {
		return new ChatOption(-1, ctx.widgets.getContinue());
	}*/
	...
	public Widget getWidget() {
		return ctx.widgets.get(WIDGET_CHAT);
	}
	...
	public List<ChatOption> getOptions() {
		...
		if (w != null) {
			for (int i = 1; i < 6; i++) {
				...
				final Component a = w.getComponent(array[0]);
				final Component b = w.getComponent(array[1]);

				if (a != null && b != null && a.isValid() && b.isValid()) {
					final String s = b.getText();

					if (s != null && s.startsWith(Integer.toString(i))) {
						list.add(new ChatOption(ctx, i, a));
						continue;
					}
				}
				break;
	 ...
	 */
	public List<ChatOption> getOptions(final String option) {
		final List<ChatOption> list = new LinkedList<>();
	 ...
	 */
	public List<ChatOption> getOptions(final Filter<ChatOption> filter) {
		final List<ChatOption> list = new LinkedList<>();
		for (final ChatOption w : getOptions()) {
			if (filter.accept(w)) {
				list.add(w);
	 ...
	 */
	public ChatOption getOption(final String option) {
	 ...
	 */
	public ChatOption getOption(final Filter<ChatOption> filter) {
