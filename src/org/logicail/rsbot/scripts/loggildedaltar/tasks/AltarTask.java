	...
	public GameObject getAltar() {
		return ctx.objects.select().id(ALTAR).sort(new Comparator<GameObject>() {
			...
			public int compare(GameObject lhs, GameObject rhs) {
				...
				return Integer.valueOf(lhsRoom.getGameObjectsInRoom(AltarLightBurnersTask.LIT_LANTERN).size()).compareTo(rhsRoom.getGameObjectsInRoom(AltarLightBurnersTask.LIT_LANTERN).size());
			}
		}).poll();
	}
