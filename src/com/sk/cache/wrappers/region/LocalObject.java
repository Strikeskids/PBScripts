...
public class LocalObject {
	...
	private final LocalObjectLoader loader;
	public final int id;
	...
	public LocalObject(LocalObjectLoader loader, int id, int lx, int ly, int plane, int type, int orientation) {
		this.id = id;
		this.x = (byte) lx;
		this.y = (byte) ly;
	...
	public ObjectDefinition getDefinition() {
		...
		if (ret == null) {
			ret = ((RT6CacheSystem) loader.getCacheSystem()).objectLoader.load(this.id);
		}
	...
	public Flagger createFlagger(Region r) {
		...
		if (type == 22 && def.blockType == 1) {
			...
			return getInteractiveFlagger(x, size.width, y, size.height, plane, def.unwalkable, !def.solid);
		} else if (0 <= type && type <= 3) {
			return getBoundaryFlagger(x, y, plane, type, orientation, def.unwalkable, !def.solid);
		}
		return null;
	}
