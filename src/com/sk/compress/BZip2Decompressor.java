...
public class BZip2Decompressor {
	...
	private static final void method1787(BZip2BlockEntry entry) {
		...
		byte abyte0[] = entry.dest;
		int i1 = entry.anInt2203;
		int j1 = entry.decompressedSize;
		int k1 = j1;
...
//		int i2 = entry.anInt2216;
	...
	private static final int method1790(int i, BZip2BlockEntry entry) {
		...
		do {
			...
			entry.anInt2207 = entry.anInt2207 << 8 | entry.source[entry.offset] & 0xff;
			entry.anInt2232 += 8;
			entry.offset++;
			entry.anInt2217++;
		} while (true);
	...
	public static final int decompress(byte destination[], int decompressedSize, byte source[], int compressedSize, int offset) {
		synchronized (entry) {
			entry.source = source;
			entry.offset = offset;
			...
			entry.anInt2217 = 0;
			entry.anInt2216 = 0;
			processDecompression(entry);
			decompressedSize -= entry.decompressedSize;
			entry.source = null;
			entry.dest = null;
			int l = decompressedSize;
			return l;
		}
	}

	private static final void processDecompression(BZip2BlockEntry entry) {
		int j8 = 0;
		int ai[] = null;
		int ai1[] = null;
		int ai2[] = null;
		entry.anInt2202 = 1;
		if (arr == null) {
			arr = new int[entry.anInt2202 * 0x186a0];
		}
		boolean flag18 = true;
		while (flag18) {
			...
			for (int k5 = ai2[k7 - ai1[l6]]; k5 != l4; ) {
				if (k5 == 0 || k5 == 1) {
					...
					arr[l5] = entry.aByteArray2211[byte6 & 0xff] & 0xff;
					l5++;
					if (j5 == 0) {
						i5++;
						j5 = 50;
			...
			for (int l2 = 0; l2 < l5; l2++) {
				byte byte7 = (byte) (arr[l2] & 0xff);
				arr[entry.anIntArray2220[byte7 & 0xff]] |= l2 << 8;
				entry.anIntArray2220[byte7 & 0xff]++;
			}

			entry.anInt2208 = arr[entry.anInt2223] >> 8;
			entry.anInt2227 = 0;
			entry.anInt2208 = arr[entry.anInt2208];
